...
const {AUTO_RESUME_DELAY} = new (require('../modules/ArjunUtils'))();
const autoJoin = require('../schemas/autoJoin');

module.exports = class AlpinePlayerHandler {
    constructor(client) {
        this.client = client;
 ...
 async autoJoin() {
  if (this.client.database == undefined) return setTimeout(() => this.autoJoin(), 1000)
        if (!this.client.database) return;
        console.log(chalk.yellow(`[LAVALINK] => [AUTO JOIN] Collecting player data`))
        const queues = await autoJoin.find()
        console.log(chalk.greenBright(`[LAVALINK] => [AUTO JOIN] found ${queues.length ? `${queues.length} queue${queues.length > 1 ? 's' : ''}. Resuming all queue` : '0 queue'}`))
        ...
        for (let data of queues) {
			...
			setTimeout(async () => {
                ...
                if (this.client.player.players.get(data.guildID) || !channel || !voice);
