  ...
  if (Array.isArray(description)) {
    try {
      ...
      for (let i = 0; i < description.length; i += 15) {
        ...
        k += 15;
        const embed = new Discord.MessageEmbed().setDescription(current).setTitle(TITLE).setColor(ee.color).setFooter(ee.footertext, ee.footericon);
        embeds.push(embed);
      }
      embeds;
    } catch {}
...
async function swap_pages2(client, message, embeds) {
  let currentPage = 0;
  if (embeds.length === 1) return message.channel.send(embeds[0]);
  queueEmbed = await message.channel.send(`**Current Page - ${currentPage + 1}/${embeds.length}**`, embeds[currentPage]);
  let reactionemojis = ["◀️", "⏹️", "▶️"];
  try {
    for (const emoji of reactionemojis) await queueEmbed.react(emoji);
  } catch {}
