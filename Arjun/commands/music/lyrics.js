...
module.exports = {
    ...
    async execute(message, args, client) {
        try {
            ...
            if (!args[0] && !player) return message.channel.send(new handler().normalEmbed(`${client.emoji.error} | Provide song name.`))
            let songTitle = args.join(' ') ? args.join(' ') : player?.queue?.current?.title;
            if (!songTitle) return message.channel.send(new handler().normalEmbed('❌ | Currently not playing anything.'))
